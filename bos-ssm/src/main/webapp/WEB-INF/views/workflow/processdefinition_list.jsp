...
<script type="text/javascript">
			...
			toolbar : [
				{
					...
					iconCls : 'icon-add',
					handler : function(){
						location.href = "${pageContext.request.contextPath}/page_workflow_processdefinition_deploy";
					}
				}
			]
		});
	...
	function del(id){
		$.messager.confirm("确认信息","你确定删除当前流程定义吗？",function(r){
			if(r){
				//发送请求，删除数据
				window.location.href = '${pageContext.request.contextPath}/processDefinitionContro/delete?id=' + id;
			}
		});
	}
</script>
  				...
  				<tr>
  					<td>
  						<%--
  						 ...
  						 --%>
  						<%-- ${id } --%>
  						<c:out value="${processDefinition.id}"></c:out>
  					</td>
  					<!-- <td><s:property value="name"/></td>
  					<td><s:property value="key"/></td>
  					<td><s:property value="version"/></td> -->
  					<td> <c:out value="${processDefinition.name}"></c:out></td>
  					<td> <c:out value="${processDefinition.key}"></c:out></td>
  					<td> <c:out value="${processDefinition.version}"></c:out></td>
  					<td>
  						<a onclick="window.open('showpng?id=${processDefinition.id}','','height=250, width=660,top=170,left=230')"
